...
class TestInferenceScalingAdapterUUIDReUse(unittest.TestCase):
    def test_extract_uuid_from_checkpoint_entry(self):
        ...
        self.assertEqual(extract_uuid_from_checkpoint_entry(entry_win), "d126cea7-9a4c-4d8f-9158-a00d93eadb25")
        ...
        self.assertEqual(extract_uuid_from_checkpoint_entry(entry_unix), "d126cea7-9a4c-4d8f-9158-a00d93eadb25")
    ...
    def test_cli_help(self):
        # Verify the --help command works and contains --reprocess-all
        res = subprocess.run(
            ["uv", "run", "python", "-m", "every_eval_ever.adapters.inference_scaling.adapter", "--help"],
            capture_output=True,
            text=True
        )
        self.assertEqual(res.returncode, 0)
        self.assertIn("--reprocess-all", res.stdout)
